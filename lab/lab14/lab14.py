def prune_min(t):
    ...
    "*** YOUR CODE HERE ***"
    if t.is_leaf(): return
    min_label, min_son = t.label, t
    for son in t.branches:
        if min_son == t or min_label > son.label:
            min_label, min_son = son.label, son
    prune_min(min_son)
    t.branches = [min_son]
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            edits, match = helper_align(skeleton_idx + 1, code_idx + 1)
            possibilities.append((skel_char + edits, match))
        # Insert
        edits, match = helper_align(skeleton_idx, code_idx + 1)
        edits, match = "+[" + code_char + "]" + edits, match + 1
        possibilities.append((edits, match))
        # Delete
        edits, match = helper_align(skeleton_idx + 1, code_idx)
        edits, match = "-[" + skel_char + "]" + edits, match + 1
        possibilities.append((edits, match))
...
def num_splits(s, d):
    ...
    "*** YOUR CODE HERE ***"
    num = 0
    def helper(s1, s2, idx):
        nonlocal num
        if idx == len(s):
            num += abs(s1 - s2) <= d
            return
        helper(s1 + s[idx], s2, idx + 1)
        helper(s1, s2 + s[idx], idx + 1)
    helper(0, 0, 0)
    return num // 2
