...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    total, minute = len(typed) / 5, elapsed / 60
    return total / minute
    # END PROBLEM 4
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for wd in valid_words:
        if user_word == wd: return wd
        num = diff_function(user_word, wd, limit)
        if num < Min: Min, ret = num, wd
    if Min > limit: ret = user_word
    return ret
    # END PROBLEM 5
...
def shifty_shifts(start, goal, limit):
    ...
    def helper(s, g):
        ...
        if len(s) > len(g):
            count += len(s) - len(g)
            return helper(s[0:len(g)], g)
        if len(g) > len(s):
            count += len(g) - len(s)
            return helper(s, g[0:len(s)])
        if s == g: return limit + 1 if count > limit else count
        if s[0] != g[0]: count += 1
        return helper(s[1: ], g[1: ])
...
def final_diff(start, goal, limit):
    ...
    return meowstake_matches(start, goal, limit)
...
def report_progress(typed, prompt, id, send):
    ...
    dict['id'], dict['progress'] = id, ratio
    send(dict)
    return ratio
    # END PROBLEM 8
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    ret = [[] for i in players]
    for i in words:
        fastest_time, fastest_player = time(game, 0, i), 0
        for j in players:
            if time(game, j, i) < fastest_time:
                fastest_time, fastest_player = time(game, j, i), j
        ret[fastest_player].append(word_at(game, i))
    return ret
    # END PROBLEM 10
...
def key_distance_diff(start, goal, limit):
    ...
    elif limit <= 0: return float('inf')
    elif start == '': return len(goal)
    elif goal == '': return len(start)
    elif start[0] == goal[0]: return key_distance_diff(start[1: ], goal[1: ], limit)
    else:
...
def faster_autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if idx in dict: return dict[idx]
    Min, ret = limit + 1, ''
    for wd in valid_words:
        num = diff_function(user_word, wd, limit)
        if num < Min: Min, ret = num, wd
