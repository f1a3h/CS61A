...
def balanced(m):
    ...
    if balanced(left_m) and balanced(right_m):
        left_t, right_t = length(left(m)) * total_weight(left_m), length(right(m)) * total_weight(right_m)
...
def totals_tree(m):
    ...
    if is_planet(m): return tree(label)
    left_m, right_m = end(left(m)), end(right(m))
    return tree(label, [totals_tree(left_m), totals_tree(right_m)])
...
def has_path(t, phrase):
    ...
    "*** YOUR CODE HERE ***"
    if len(phrase) == 1: return label(t) == phrase
    if label(t) == phrase[0]:
        flag = False
        for son in branches(t):
            if has_path(son, phrase[1: ]): flag = True
        return flag
    else: return False
...
def quadratic(x, a, b, c):
    ...
    k, m = -b / (2 * a), f(-b / (2 * a))
    if a > 0:
        if x1 <= k <= x2: return interval(m, max(f1, f2))
        else: return interval(min(f1, f2), max(f1, f2))
    else:
        if x1 <= k <= x2: return interval(min(f1, f2), m)
        else: return interval(min(f1, f2), max(f1, f2))
