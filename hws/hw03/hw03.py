...
def g_iter(n):
    ...
    for i in range(1, n + 1):
        if i <= 3: ans[i] = i
        else: ans[i] = ans[i - 1] + ans[i - 2] * 2 + ans[i - 3] * 3
    return ans[n]
...
def missing_digits(n):
    ...
    if n_str[0] == n_str[1]: tmp = 0
    else: tmp = int(n_str[1]) - int(n_str[0]) - 1
    if length == 2:
        return tmp
    else:
        return tmp + missing_digits(int(n_str[1: ]))
...
def count_change(total):
    ...
    def helper(total, max_coin):
        ...
        elif max_coin == 0: return 0
        else: return helper(total - max_coin, max_coin) + helper(total, max_coin >> 1)
    def max_coin(total):
        if total == 0: return 0
        elif total == 1: return 1
        else: return 2 * max_coin(total >> 1)
    return helper(total, max_coin(total))
