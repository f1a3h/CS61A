...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    balance, inven_num = 0, 0

    def __init__(self, product, cost):
        self.product, self.cost = product, cost

    def vend(self):
        if self.inven_num == 0: return 'Inventory empty. Restocking required.'
    ...
    def restock(self, num):
        self.inven_num += num
        return 'Current {0} stock: {1}'.format(self.product, self.inven_num)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        passed = Mint.current_year - self.year
        if passed > 50: return self.cents + (passed - 50)
...
def is_bst(t):
    ...
    "*** YOUR CODE HERE ***"
    def bst_max(tr):
        if tr.is_leaf(): return tr.label
        if len(tr.branches) == 1:
            if tr.label < tr.branches[0].label:
                return bst_max(tr.branches[0])
            else:
                return tr.label
        else:
            return bst_max(tr.branches[0])
    ...
    def bst_min(tr):
        ...
        if len(tr.branches) == 1:
            if tr.label > tr.branches[0].label:
                return bst_min(tr.branches[0])
            else:
                return tr.label
        else:
            return bst_min(tr.branches[1])

    if t.is_leaf(): return True
    if len(t.branches) > 2: return False
    if len(t.branches) == 2:
        ls, rs = t.branches
        return bst_max(ls) <= t.label < bst_min(rs) and is_bst(ls) and is_bst(rs)
    else:
...
def store_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    if n == 0: return Link.empty
    k, num = n, 1
    while k > 9:
        k = k // 10
        num *= 10
    return Link(k, store_digits(n - k * num))
...
def path_yielder(t, value):
    ...
    # print("DEBUG: ", t.label)
    if t.is_leaf():
        if t.label == value: yield [t.label]
        else: yield []
        return
    if t.label == value: yield [t.label]

    for son in t.branches:
        for path in path_yielder(son, value):
            if path == []: continue
            print("DEBUG: ", path)
            path = [t.label] + path
            if value in path:
...
def deep_map(f, link):
    ...
    """
    "*** YOUR CODE HERE ***"
    if link == Link.empty: return link
