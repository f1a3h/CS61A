...
def in_order_traversal(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    if t.is_leaf(): yield t.label
    else:
        it =  iter(in_order_traversal(t.branches[0]))
        nxt = next(it, None)
        while nxt != None:
            yield nxt
            nxt = next(it, None)
        yield t.label
        it = iter(in_order_traversal(t.branches[1]))
        nxt = next(it, None)
        while nxt != None:
            yield nxt
            nxt = next(it, None)
...
def interleaved_sum(n, odd_term, even_term):
    ...
    if n == 0: return 0
    def helper(n):
        if n == 0: return even_term, 0
        if n == 1: return odd_term, odd_term(1)
        term, s = helper(n - 2)
        s += term(n)
        return term, s
    t1, s1 = helper(n)
